...
public class RemediationInsertTest {
    ...
    private void testResultTests (CompleteTestResults expectedTestResult, TestResultDynamo actualTestResult) {
        ...
        //HGV Specific Tests
        if (Objects.equals(expectedTestResult.getVehicleType().getValue(), "hgv")) {
            odometerTests(expectedTestResult, actualTestResult);
        }
        //TRL Specific Tests
        if (Objects.equals(expectedTestResult.getVehicleType().getValue(), "trl")) {
            trailerIdTests(expectedTestResult, actualTestResult);
        }
        //PSV Specific Tests
        if (Objects.equals(expectedTestResult.getVehicleType().getValue(), "psv")) {
            odometerTests(expectedTestResult, actualTestResult);
...
//        expiryDateTests(expectedTestType, actualTestResult);
        //TODO add in TRL specific tests
        //TRL specific tests
//        if (Objects.equals(expectedVehicleType, "trl")){
//        }
        //PSV Specific Tests
        //TODO Test with PSV records/results
        if (Objects.equals(expectedTestResult.getVehicleType().getValue(), "psv")) {
            seatbeltInstallationTests(expectedTestType, actualTestResult);
            // Not migrated in current sample
...
//            modificationTypeUsedTests(expectedTestType, actualTestResult);
    ...
    //not currently used as expiry dates are re-generated on migration
...
//    }
    ...
    private void seatbeltInstallationTests(TestTypeResults expectedTestType, TestResultDynamo actualTestResult) {
        ...
        if (expectedLastSeatbeltInstallationCheckDate != null){
            expectedLastSeatbeltInstallationCheckDateString = expectedLastSeatbeltInstallationCheckDate.format(DateTimeFormatter.ofPattern("uuuu-MM-dd", Locale.ENGLISH));
        }
    ...
    //not currently used for sample of test migrated
